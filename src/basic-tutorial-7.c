...
tutorial_main (int argc, char *argv[])
{
  ...
  // tee_factory = gst_element_factory_find ("tee");
  ...
  tee_audio_pad = gst_element_request_pad (tee,templ,NULL,NULL);
  g_print ("Obtained request pad %s for audio branch.\n",
      gst_pad_get_name (tee_audio_pad));
  queue_audio_pad = gst_element_get_static_pad (audio_queue, "sink");
  tee_video_pad = gst_element_request_pad (tee, templ,NULL,NULL);
  ...
  g_print("Lets block the loop\n");
  msg =
      gst_bus_timed_pop_filtered (bus, GST_CLOCK_TIME_NONE,
      GST_MESSAGE_ERROR | GST_MESSAGE_EOS);

  g_print("Lets release the loop\n");
