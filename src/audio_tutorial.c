...
{ 
  ...
  GstElement *audio_conv1, *audio_conv2,*audio_decoder, *audio_resample, *audio_encoder, *audio_sink; 
...
//     filename = argv[1];
  ...
  audio_conv1 = gst_element_factory_make ("audioconvert", "audioconverter1");
  audio_conv2 = gst_element_factory_make ("audioconvert", "audioconverter2");
  ...
  if (!pipeline || !source || !demuxer || !video_parser || !video_decoder || !video_sink ||!filesink ||!muxer ||!Q.aqueue ||!Q.vqueue||
        !audio_encoder ||!audio_decoder || !audio_conv1 ||!identity ) { 
  ...
  g_object_set(G_OBJECT(filesink),"location","/home/jafar/lon_audio.mkv",NULL);
  ...
  // int aa = gst_pad_set_caps (vsrcpad, newcaps);
  // g_print("aa: %d\n",aa);
  // if (!aa) {
  ...
  gst_bin_add_many (GST_BIN (pipeline), source, demuxer, Q.aqueue, audio_decoder,typefind,audio_conv1, audio_conv2,audio_encoder,muxer,identity,filesink, NULL);
  ...
  /* Link audio elements */
  // if(gst_element_link_many (Q.aqueue,audio_decoder,audio_conv,typefind,audio_encoder,NULL)!=TRUE)
  // {
  ...
  /* Link audio elements */
  if(gst_element_link_many (Q.aqueue,audio_decoder,audio_conv1,audio_encoder,typefind,identity,NULL)!=TRUE)
  {
    g_printerr ("Elements could not be linkedin audio pipeline.\n");
        gst_object_unref (pipeline);
        return -1;
  ...
  GstPadTemplate *au_templ;
  GstPad *asrcpad, *mux_audio_pad;
  ...
  // asrcpad = gst_element_get_static_pad (audio_encoder, "src");
  ...
  au_templ = gst_element_class_get_pad_template(GST_ELEMENT_GET_CLASS(muxer), "sink_%d");
