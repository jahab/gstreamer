...
//   g_print("probe type %d \n",info->type);
  ...
  // new_buffer = gst_buffer_new_allocate (NULL, gst_buffer_get_size(buff), NULL);
  // gst_buffer_map (new_buffer, &map, GST_MAP_WRITE);

  // memcpy( (GstBuffer *)map.data, buff,  gst_buffer_get_size( new_buffer ) );
  ...
  if (buflen==100)
  {
    ...
    // GstPad *srccpad = gst_element_get_static_pad(videoconvert,"sink");
    // gst_pad_push_list(srccpad,copy_buflist);
    GstFlowReturn retval = gst_app_src_push_buffer_list((GstAppSrc*)appsrc,copy_buflist);
    g_print("RETVAL %d\n", retval);
    g_print("Sending EOS!!!!!!!");
    g_signal_emit_by_name (appsrc, "end-of-stream", &retval);

    // gst_pad_send_event(srccpad, gst_event_new_eos()); 
...
//   g_print("RETVAL %d\n", retval);
  ...
  // g_return_val_if_fail (gst_buffer_is_writable (buff), FALSE);
  g_print("writable buffer %d\n",gst_buffer_is_writable (buff));
  // gst_clear_buffer (&buff);


  
  // g_signal_emit_by_name (appsrc, "push-buffer", buff, &retval);
  // retval = gst_app_src_push_buffer((GstAppSrc*)appsrc,buff);
  
  // new_buffer = gst_buffer_new_and_alloc (gst_buffer_get_size( buff ));
  // gst_buffer_copy_into(new_buffer,buff,GST_BUFFER_COPY_DEEP,0,gst_buffer_get_size( buff ));
  // g_signal_emit_by_name (appsrc, "push-buffer", new_buffer, &retval);
  // gst_buffer_list_add (buflist, new_buffer);


  // new_buffer = gst_buffer_new_allocate (NULL, 1920*1080*3, NULL);
  // new_buffer = gst_buffer_new_wrapped(buff,1920*1080*2);
...
int tutorial_main(int argc, char *argv[])
{
    ...
    gst_bin_add_many (GST_BIN (pipeline), video_src,videoconvert,videoencode,muxer, typefind,identity1,identity2,fake_sink, NULL);
    if ((gst_element_link_many(video_src,videoconvert,videoencode,muxer,identity1, typefind,identity2, fake_sink, NULL) != TRUE ))  {
        g_printerr ("Elements could not be linked.\n");
        gst_object_unref (pipeline);
        return -1;
      }
    ...
    gst_bin_add_many (GST_BIN (pipeline2), appsrc,typefind,file_sink2,identity3, NULL);
