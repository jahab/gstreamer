...
//   gst_buffer_list_get_writable(buflist);
...
int tutorial_main(int argc, char *argv[])
{
    ...
    gst_bin_add_many (GST_BIN (pipeline), video_src,identity1,identity2,fake_sink, NULL);
    if ((gst_element_link_many(video_src,identity1, identity2, fake_sink, NULL) != TRUE ))  {
        g_printerr ("Elements could not be linked.\n");
        gst_object_unref (pipeline);
        return -1;
      }
