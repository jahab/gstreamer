...
//     filename = argv[1];
  ...
  g_object_set(G_OBJECT(filesink),"location","/home/jafar/lon.mkv",NULL);
  ...
  // GstPad *ratesrcpad = gst_element_get_static_pad (audio_rate, "src");

  // g_object_set (ratesrcpad, "caps",
  ...
  // gst_bin_add_many (GST_BIN (pipeline), source, demuxer, video_parser, video_decoder, video_sink, Q.vqueue,
  //                   Q.aqueue, audio_decoder, audio_conv, audio_encoder,audio_sink, NULL);
  ...
  gst_bin_add_many (GST_BIN (pipeline), source, demuxer, Q.vqueue, video_parser, video_decoder, video_convert,video_encoder,
                    Q.aqueue, audio_decoder, audio_conv,audio_encoder,muxer,audio_resample,identity, filesink, NULL);
  ...
  // /* Link audio elements */
  // if(gst_element_link_many (Q.aqueue,audio_decoder,audio_conv,audio_encoder,typefind,identity,NULL)!=TRUE)
  // {
  ...
  /* Link audio elements */
  if(gst_element_link_many (Q.aqueue,audio_decoder,audio_conv,audio_encoder,identity,NULL)!=TRUE)
  {
    g_printerr ("Elements could not be linkedin audio pipeline.\n");
        gst_object_unref (pipeline);
        return -1;
