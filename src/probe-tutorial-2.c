...
fake_event_probe_cb (GstPad * pad, GstPadProbeInfo * info, gpointer user_data)
{
    ...
    g_print("GST EVENT TYPE on FINAL VIDEO SINK %d %d\n", GST_EVENT_EOS,GST_EVENT_TYPE (GST_PAD_PROBE_INFO_DATA (info)));  
    if (GST_EVENT_TYPE (GST_PAD_PROBE_INFO_DATA (info)) != GST_EVENT_EOS)
      return GST_PAD_PROBE_REMOVE;
    // g_usleep(1000000);
    system("cp /home/jafar/gs.mp4 /home/jafar/gss.mp4");
    // system("ffmpeg -y -i /home/jafar/gs.mp4 -c copy /home/jafar/gss.mp4");
...
//   gst_pad_add_probe (srcpad, GST_PAD_PROBE_TYPE_BLOCK , event_probe_cb, user_data, NULL);
  gst_element_set_state (identity, GST_STATE_NULL);
  gst_element_set_state (identity, GST_STATE_PLAYING);
...
int tutorial_main(int argc, char *argv[])
{
    ...
    // g_object_set(videoencode, "tune", 4, NULL);
